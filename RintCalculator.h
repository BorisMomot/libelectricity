...
public:
    ...
    virtual double calculateRinternal(std::chrono::milliseconds dTime, double Pcurrent, double Ftarget, double Fcurrent, double Pkr=0);
    virtual double calculatesinFi(std::chrono::milliseconds dTime, double Pcurrent, double Ftarget, double Fcurrent, double Pkr=0);//функция для расчета угла между магнитным полем ротора и статора
protected:
